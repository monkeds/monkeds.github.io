...
function putColorPaletteInHTML(indicator, pantonePalette){
    $(indicator+" .colors").html("");
    for(var i = 0; i< pantonePalette.length ;i++){
        $(indicator+" .colors").append(`<div class="col-sm" style="background-color:#`+pantonePalette[i]['hex']+`">
                                        `+pantonePalette[i]['name']+`
                                    </div>`)
    }
}
...
function putColorToBackground(indicator, pantonePalette, cantColors){
    if(_colorFondo == 'R'){
        if(randomTrueFalse()){
            putSolidColor(indicator+">.tshirt-wrap>.layer_fondo", pantonePalette[0]);
        }else{
            putGradientColor(indicator+">.tshirt-wrap>.layer_fondo", pantonePalette, cantColors);
        }
...
function putColorToNeck(indicator, pantoneColor){
    if(_colorCuello == 'R'){
        if(randomTrueFalse()){
            putSolidColor(indicator+">.tshirt-wrap>.layer_cuello", pantoneColor);
        }
...
function putColorShortSleve(indicator,  pantoneColor){
    putSolidColor(indicator+">.tshirt-wrap>.layer_mangas", pantoneColor);
}
function putColorLongSleve(indicator,  pantoneColor){
    putSolidColor(indicator+">.tshirt-wrap>.layer_mangas_altas", pantoneColor);
...
function generatePoloTemplate(id){
    var template=`<div id=`+id+` class="card" style="max-width: 402px;">
        <div  class="tshirt-wrap polo card-img-top">
...
function generateCamiseroTemplate(id){
    var template=`<div id=`+id+` class="card" style="max-width: 402px;">
        <div  class="tshirt-wrap polo-camisero card-img-top">
